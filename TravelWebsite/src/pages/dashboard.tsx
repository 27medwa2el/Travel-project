...
import { tripStore, cityStore, activityStore, eventStore, seedMockData } from '@/lib/mockStore';
import { Trip, City, Activity, CityEvent } from '@/types/domain';
import { getAuth } from '@clerk/nextjs/server';
import CityMapView from '../components/CityMapView';
import { cn } from '@/lib/utils';

const Dashboard = ({ trips = [], allCities = [], allActivities = [], allEvents = [] }: { 
  trips: Trip[], 
  allCities: City[], 
  ...
  const tripActivities = React.useMemo(() => {
    ...
    return activities;
  }, [selectedTrip, allActivities]);

  const mapCenter = React.useMemo(() => {
    if (tripActivities.length > 0) {
      return { lat: tripActivities[0].lat, lng: tripActivities[0].lng };
    }
                  ...
                  <div className="relative h-[300px] rounded-[50px] overflow-hidden shadow-2xl">
                    ...
                    <div className="absolute top-10 right-10 flex gap-2">
                      <button 
                        ...
                        className={cn(
                          "px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center gap-2",
                          view === 'itinerary' ? "bg-white text-gray-900 shadow-xl" : "bg-white/10 backdrop-blur-md text-white border border-white/20 hover:bg-white/20"
                        )}
                      >
                        <CalendarDaysIcon className="w-4 h-4" /> Itinerary
                      </button>
                      <button 
                        onClick={() => setView('map')}
                        className={cn(
                          "px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center gap-2",
                          view === 'map' ? "bg-white text-gray-900 shadow-xl" : "bg-white/10 backdrop-blur-md text-white border border-white/20 hover:bg-white/20"
                        )}
                      >
                        <GlobeAltIcon className="w-4 h-4" /> Map View
                      </button>
                    </div>
                                      ...
                                      <div key={item.id} className="bg-gray-50 p-6 rounded-[30px] border border-gray-100 group hover:border-purple-200 transition-all">
                                        <div className="flex items-center gap-4 mb-4">
                                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white ${item.type === 'ACTIVITY' ? 'bg-blue-500' : 'bg-purple-500'}`}>
                                            {item.type === 'ACTIVITY' ? <SparklesIcon className="w-5 h-5" /> : <CalendarDaysIcon className="w-5 h-5" />}
                                          </div>
                                          <div>
